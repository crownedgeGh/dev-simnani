 ...
 *
 * Pass { searchTermsOnly: true } to skip every other report — used when the
 * admin dashboard is just re-filtering Top Search Queries by state/city and
 * doesn't need to re-fetch the KPI cards, trend chart, etc. */
export async function getDashboardReports({ searchTermsOnly = false } = {}) {
  ...
  const allJobs = {
    ...
    // limit is intentionally high (not the 10 the panel displays) — the
    // admin dashboard slices this down to the top 10 *after* filtering by
    // state/city, so it needs the full recent spread of queries to filter.
    topSearchTerms: () =>
      analyticsDataClient.runReport({
        property,
        dateRanges: [{ startDate: "28daysAgo", endDate: "today" }],
  ...
  const jobs = searchTermsOnly ? { topSearchTerms: allJobs.topSearchTerms } : allJobs;

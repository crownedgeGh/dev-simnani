...
// /industrial and /invest actually render.
...
// Residential section.
export const CATEGORIES_WITH_BEDROOMS = new Set(["farm-house", "farmhouse", "apartments"]);
...
// types (buy/sell/rent/lease/seized-property) are never in CATEGORIES_BY_TYPE
// and always represent a structure.
...
export function categoryHasBedrooms(type, category) {
  if (!CATEGORIES_BY_TYPE[type]) return true;
...
export function getPropertyCategoryLabels(property) {
  ...
  if (["buy", "sell", "rent", "lease", "seized"].includes(property?.type)) {

...
// ---------------------------------------------------------------------------
// CP leads — leads forwarded to Head CP by a Digital CP or Field CP land
// here, prepended, so they show up immediately on /admin/freelancer-cp/head-cp.
// ---------------------------------------------------------------------------
export function addCpLead(lead) {
  const leads = readCollection(ADMIN_KEYS.cpLeads) || [];
  writeCollection(ADMIN_KEYS.cpLeads, [lead, ...leads]);
}
...
// ---------------------------------------------------------------------------
// CP leads used to ship with hardcoded demo entries. This flag lets us purge
// any dummy data already sitting in a returning browser's localStorage once,
// so the collection is populated only by real "Forward to Head CP" submissions.
// We only strip the known dummy IDs (rather than wiping the collection) so a
// real lead forwarded before the admin panel was ever opened survives.
const CP_LEADS_DUMMY_DATA_PURGED_KEY = "se_admin_cp_leads_purged_v1";
const DUMMY_CP_LEAD_IDS = new Set(CP_LEADS.map((l) => l.id));
...
export function seedAdminData() {
  ...
  if (!localStorage.getItem(CP_LEADS_DUMMY_DATA_PURGED_KEY)) {
    const existingCpLeads = readCollection(ADMIN_KEYS.cpLeads);
    if (existingCpLeads) {
      writeCollection(ADMIN_KEYS.cpLeads, existingCpLeads.filter((l) => !DUMMY_CP_LEAD_IDS.has(l.id)));
    }
    localStorage.setItem(CP_LEADS_DUMMY_DATA_PURGED_KEY, "1");
  }
